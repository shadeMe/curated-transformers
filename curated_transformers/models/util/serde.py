...
# Args: Parent module, parameter name, tensor to convert.
# Returns the new paramater
TensorToParameterConverterT = Callable[[Module, str, torch.Tensor], Parameter]
...
def default_tensor_to_parameter_converter(
    module: Module, parameter_name: str, tensor: torch.Tensor
) -> Parameter:
    ...
    return Parameter(tensor, requires_grad=old_param.requires_grad)
...
def _emplace_module_tensor(
    ...
    if device is not None:
        replacement_tensor = replacement_tensor.to(device=device)

    full_name = f"{module_prefix}{tensor_name}"
    if is_parameter:
        old_param = module._parameters[tensor_name]
        assert old_param is not None
        _validate_replacement(old_param, replacement_tensor, full_name)
        new_param = tensor_to_param_converter(module, tensor_name, replacement_tensor)
        module._parameters[tensor_name] = new_param
    else:
        old_buffer = module._buffers[tensor_name]
        assert old_buffer is not None
        _validate_replacement(old_buffer, replacement_tensor, full_name)
